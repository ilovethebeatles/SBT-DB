...
![image](https://github.com/ilovethebeatles/SBT-DB/assets/106533857/a40c8ce2-c491-4c89-9c1c-568d7edc03ff)
...
В Yellowbrick используется горизонтальный шардинг. Когда данные записываются в Yellowbrick, строки табличных данных распределяются между узлами кластера. Эти данные организуются в множественные разделенные единицы(шарды). Каждый шард представляет собой около 200 МБ сжатых данных. Внутри каждого шарда данные укладываются в колоночном виде. Это означает, что данные из отдельных колонок могут быть прочитаны и обработаны без необходимости чтения всего шарда целиком. В Yellowbrick поддерживается кластеризация данных по колоночным ключам(до 4 колоночных ключей). Это позволяет группировать связанные строки, улучшая эффективность доступа к данным для определенных типов запросов. Например, если запросы часто выбирают строки по времени и по клиенту, кластеризация по обеим колонкам сделает доступ к таким данным более эффективным.

При создании шардов Yellowbrick строит гранулированные индексы и сжимает данные. Система использует несколько техник сжатия, с акцентом на эффективность распаковки данных на CPU. Индексы хранят общие уникальные значения, структуры данных для вычисления статистики кардинальности и диапазоны значений. 

При удалении данных Yellowbrick удаляет строки в колоночном хранилище, записывая идентификаторы удаленных строк в новые файлы рядом с шардами. Обновления строк обрабатываются путем удаления старых строк и вставки новых в новый шард. Встроенный сборщик мусора периодически очищает фрагментированные шарды, удаляет удаленные строки и эффективно их рекомбинирует. Это происходит инкрементально, в очень малых единицах хранения, что позволяет избежать заметного влияния на производительность.
...
Yellowbrick создана с предположением, что все по умолчанию является "приватным". Контроль доступа к различным областям продукта ограничен. Частные S3-бакеты включены по умолчанию без публичного доступа к данным. Встроенные учетные записи гостевых пользователей отсутствуют, и строгий доступ должен быть предоставлен ко всем данным и функциям управления. Поскольку Yellowbrick Data Warehouse работает в собственном облачном аккаунте клиента, Yellowbrick по умолчанию не имеет доступа к данным клиента, запросам клиента или журналам. Компании полностью контролируют, кто имеет доступ и к чему он может получить доступ.
...
У Yellowbrick множкство инвесторов, одни из самых известных - BMW и Samsung.
